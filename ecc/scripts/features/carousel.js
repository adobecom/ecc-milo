...
export async function onCarouselCSSLoad(selector, parent, options) {
...
export default async function buildCarousel(selector, parent, options = {}) {
  const { loadStyle, getConfig } = await import(`${LIBS}/utils/utils.js`);
  // Load CSS then build carousel
  return new Promise((resolve) => {
    loadStyle(`${getConfig().codeRoot}/scripts/features/carousel.css`, async () => {
      await onCarouselCSSLoad(selector, parent, options);
      resolve();
    });
  });
}
