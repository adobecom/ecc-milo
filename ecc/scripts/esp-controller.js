...
export async function getEventByExternalId(eventExternalId) {
  const { host } = API_CONFIG.esp[getCurrentEnvironment()];
  const options = await constructRequestOptions('GET');

  try {
    const response = await safeFetch(`${host}/v1/external/events/${eventExternalId}`, options);
    const data = await response.json();

    if (!response.ok) {
      window.lana?.log(`Failed to get event by external ID ${eventExternalId}. Status: ${response.status}\nError: ${JSON.stringify(data, null, 2)}`);
      return { status: response.status, error: data };
    }

    return data;
  } catch (error) {
    window.lana?.log(`Failed to get event by external ID ${eventExternalId}:\n${JSON.stringify(error, null, 2)}`);
    return { status: 'Network Error', error: error.message };
  }
}
