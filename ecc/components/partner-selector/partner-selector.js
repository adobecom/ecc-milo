...
export default class PartnerSelector extends LitElement {
  ...
  this.updateValue('link', event.target.value);
      ...
      <div class="action-area">
        <sp-button variant="primary" .disabled=${!this.partner.name.length >= 3 || !this.partner.link.match('^https:\\/\\/[a-z0-9]+([\\-\\.]{1}[a-z0-9]+)*\\.[a-z]{2,5}(:[0-9]{1,5})?(\\/.*)?$')} class="save-partner-button" @click=${this.savePartner}>Save Partner</sp-button>
