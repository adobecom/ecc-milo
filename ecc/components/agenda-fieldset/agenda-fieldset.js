...
export default class AgendaFieldset extends LitElement {
      ...
      <div class="field-container">
        <div class="time-picker">
          ...
          <div class="time-picker-wrapper">
            <sp-picker required="true" class="time-picker-input select-input" label="Pick agenda time" value=${this.agenda.startTime} @change=${(event) => {
  this.updateValue('startTime', event.target.value);
        ...
        <div class="text-field-wrapper">
          <sp-textfield required="true" class="text-input" placeholder=${this.options.placeholder} value=${this.agenda.description} ?required=${this.options.isRequired && (this.agendas.length > 1 || this.agenda.startTime)} quiet size="xl" maxlength=${this.options.maxCharNum} @change=${(event) => {
