...
// TODO: remove and use USP instead
const TEST_EVENT_ID = 'dee89ecc-0f3a-4a8d-a0d1-3037df512181';
...
async function getAttendeesArray() {
  const resp = await getAttendees(TEST_EVENT_ID);
...
async function buildDashboard(el, config) {
  ...
  const data = await getAttendeesArray();
  if (!data?.length) {
...
function buildLoadingScreen(el) {
  ...
  createTag('sp-field-label', {}, 'Loading Adobe Event Creation Console Attendee Management...', { parent: loadingScreen });
