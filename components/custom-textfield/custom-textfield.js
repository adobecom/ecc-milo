...
export class CustomTextfield extends LitElement {
  ...
  render() {
    return html`
    <sp-textfield placeholder=${this.data.placeholder} quiet size=${this.config.size} ?grows=${this.config.grows} ?multiline=${this.config.multiline} class='text-input' value=${this.data.value} @change=${(event) => this.dispatchEvent(new CustomEvent('input-change', { detail: { value: event.target.value } }))}></sp-textfield>
